              ...
              {[
                    ...
                    <div className="space-y-2">
                      {eventTypes.map((type) => (
                        <label key={type} className="flex items-center gap-3 cursor-pointer">
                          <input type="checkbox" checked={filters.type === type.toLowerCase()} onChange={(e) => handleFilterChange("type", e.target.checked ? type.toLowerCase() : "")} className="w-4 h-4 rounded" />
                ...
                return (
                  <div key={filter.id} className="border-b border-white/10 pb-4">
                    <button onClick={() => toggleFilterSection(filter.id)} className="flex items-center justify-between w-full font-semibold text-sm">
                      <span className="flex items-center gap-2"><Icon className="w-4 h-4" />{filter.title}</span>
                      <ChevronDown className={`w-4 h-4 transition-transform ${isExpanded ? "rotate-180" : ""}`} />
                    </button>
                    {isExpanded && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3">{filter.content}</motion.div>}
                  </div>
                );
              })}
              <Button variant="secondary" size="md" className="w-full" onClick={() => setFilters({ type: "", dateFrom: "", dateTo: "", priceMin: 0, priceMax: 1000 })}>Réinitialiser</Button>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="lg:col-span-3">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-2xl font-bold">{filteredResults.length} événements trouvés</h2>
            </div>
