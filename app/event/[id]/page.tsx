...
function PackSidebar({
  ...
  return (
    <div className="bg-white border border-black/[0.08] rounded-2xl p-6 sticky top-24 shadow-sm">
      <h3 className="font-bold text-lg text-[#1d1d1f] mb-4 flex items-center gap-2">
        <Zap className="w-5 h-5 text-blue-400" /> Votre Pack
      </h3>

      {items.length === 0 ? (
        <p className="text-[#86868b] text-sm text-center py-6">
        ...
        <div className="space-y-3 mb-4">
          {items.map((item, i) => (
            <div key={i} className="flex justify-between text-sm">
              <span className="text-[#6e6e73] capitalize flex items-center gap-1.5">
                ...
                {item.type === "ticket" && (item.quantity || 1) > 1 && <span className="text-[#86868b]">×{item.quantity}</span>}
              </span>
          ...
          <div className="flex justify-between text-sm text-[#86868b] pt-2 border-t border-black/[0.06]">
...
export default function EventDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#86868b]">Chargement de l'événement…</p>
        </div>
      </div>
    );
  }
  ...
  if (!event) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <Zap className="w-16 h-16 text-[#d2d2d7] mx-auto mb-4" />
          <h1 className="text-2xl font-bold text-[#1d1d1f] mb-2">Événement introuvable</h1>
          <p className="text-[#86868b] mb-6">Cet événement n'est plus disponible.</p>
  ...
  const steps = [
      ...
      content: (
        <div className="space-y-4">
          ...
          {soldOut && (
            <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#86868b] mt-0.5">Plus aucun billet disponible via cet événement. Consultez Ticketmaster pour les reventes.</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 mb-2">
            <span className="text-sm font-semibold text-[#6e6e73]">Personnes :</span>
            <div className="flex items-center gap-2">
              <button onClick={() => setTicketQty(q => Math.max(1, q - 1))}
                className="w-8 h-8 rounded-full border border-black/[0.08] flex items-center justify-center hover:bg-[#f0f0f5] text-[#6e6e73] transition-colors">
                <Minus className="w-4 h-4" />
              </button>
              <span className="font-bold text-[#1d1d1f] w-6 text-center">{ticketQty}</span>
          ...
          <div className="border border-black/[0.08] rounded-xl overflow-hidden max-h-[420px] overflow-y-auto">
            {cats.map((cat, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3.5 text-left transition-all border-b border-black/[0.06] last:border-b-0 ${
                    isSoldOut
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <span className="font-bold text-[#1d1d1f] text-sm uppercase tracking-wide truncate">
                      {cat.name}
                    </span>
                    {cat.badge && (
                  ...
                  <div className="text-right flex-shrink-0 ml-4">
                    <span className="font-bold text-[#1d1d1f]">{formatPrice(cat.price)}</span>
                    {ticketQty > 1 && (
                      <div className="text-[11px] text-[#86868b]">× {ticketQty} = {formatPrice(cat.price * ticketQty)}</div>
                    )}
                  </div>
                </button>
              );
          ...
          <div className="mt-4 pt-4 border-t border-black/[0.06] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
    ...
    // ── HÉBERGEMENT ───────────────────────────────────────────────────────────
    {
      ...
      content: (
        <div className="space-y-4">
          <div className="flex items-center gap-4 mb-2">
            <span className="text-sm font-semibold text-[#6e6e73]">Nuits :</span>
            <div className="flex items-center gap-2">
              <button onClick={() => setNights(n => Math.max(1, n - 1))}
                className="w-8 h-8 rounded-full border border-black/[0.08] flex items-center justify-center hover:bg-[#f0f0f5] text-[#6e6e73] transition-colors">
                <Minus className="w-4 h-4" />
              </button>
              <span className="font-bold text-[#1d1d1f] w-6 text-center">{nights}</span>
          ...
          {hotelsLoading ? (
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border-2 text-left overflow-hidden transition-all ${
                      isSelected ? "border-blue-500 bg-blue-500/10" : "border-black/[0.08] hover:border-white/30"
                    }`}
                  >
                    {hotel.image && (
                      <div className="h-28 overflow-hidden bg-[#f5f5f7]">
                        <img src={hotel.image} alt={hotel.name} className="w-full h-full object-cover opacity-80" />
                      </div>
                    )}
                    <div className="p-4">
                      <div className="font-bold text-[#1d1d1f] text-sm mb-1 line-clamp-1">{hotel.name}</div>
                      <div className="flex items-center gap-1 mb-1">
                        {Array.from({ length: hotel.stars || 3 }).map((_, i) => (
                          <Star key={i} className="w-3 h-3 text-yellow-400 fill-yellow-400" />
                        ))}
                        {hotel.rating && <span className="text-xs text-[#86868b] ml-1">{hotel.rating}/10</span>}
                      </div>
                      <div className="flex items-center gap-1 text-xs text-[#86868b] mb-2">
                        <MapPin className="w-3 h-3" />{hotel.distance?.toFixed(1) || "?"} km du lieu
                      </div>
                      <div className="font-bold text-blue-400">{formatPrice(hotel.price)}<span className="text-xs text-[#86868b] font-normal">/nuit</span></div>
                      <div className="text-xs text-[#86868b]">× {nights} nuits = {formatPrice(hotel.price * nights)}</div>
            ...
            <div className="text-center py-10 text-[#86868b]">
              ...
              <p className="text-xs mt-1 text-[#d2d2d7]">Hôtels sélectionnés près de {event.venue}</p>
            </div>
          )}
        </div>
      ),
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-[#86868b] mb-1.5 flex items-center gap-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-black/[0.08] rounded-xl text-sm text-[#1d1d1f] focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30 [color-scheme:dark]"
              />
            </div>
            <div>
              <label className="text-xs text-[#86868b] mb-1.5 flex items-center gap-1.5">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <PlaneTakeoff className="absolute left-3 top-3 w-4 h-4 text-[#86868b] pointer-events-none z-10" />
              <button
                type="button"
                onClick={() => setShowCityMenu(v => !v)}
                className={`w-full pl-9 pr-8 py-2.5 bg-[#f5f5f7] border rounded-xl text-sm text-left transition-colors focus:outline-none ${
                  showCityMenu ? "border-blue-500/50 ring-1 ring-blue-500/30" : "border-black/[0.08] hover:border-white/20"
                } ${!departureCity ? "text-[#86868b]" : "text-[#1d1d1f]"}`}
              >
                {departureCity || "D'où partez-vous ?"}
              </button>
              <ChevronDown className={`absolute right-3 top-3 w-4 h-4 text-[#86868b] pointer-events-none transition-transform ${showCityMenu ? "rotate-180" : ""}`} />

              {/* Dropdown menu */}
              {showCityMenu && (
                <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-white border border-black/[0.08] rounded-xl shadow-2xl overflow-hidden"
                  style={{ maxHeight: "280px", overflowY: "auto" }}
                >
                  {DEPARTURE_CITIES.map(group => (
                    <div key={group.country}>
                      <p className="px-4 pt-3 pb-1 text-xs font-semibold text-[#86868b] uppercase tracking-wide">
                        ...
                        {group.cities.map(city => (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2 text-sm transition-colors hover:bg-blue-500/10 hover:text-blue-300 ${
                              departureCity === city ? "text-blue-300 bg-blue-500/10 font-medium" : "text-[#6e6e73]"
                            }`}
                          >
                            {city}
                          </button>
          ...
          {flightsLoading ? (
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border-2 p-4 text-left transition-all ${
                      isSelected ? "border-blue-500 bg-blue-500/10" : "border-black/[0.08] hover:border-white/30 hover:bg-[#f5f5f7]"
                    }`}
                  >
                    {/* Ligne 1 : compagnie + prix */}
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-[#1d1d1f]">{flight.airline}</span>
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30">A/R</span>
                        <span className={`text-xs font-medium ${flight.stops === 0 ? "text-green-400" : "text-[#86868b]"}`}>
                      ...
                      <div className="text-right flex-shrink-0 ml-4">
                        ...
                        <div className="text-[11px] text-[#86868b]">{ticketQty} pers. · A/R</div>
                      </div>
                    </div>

                    {/* Ligne 2 : ALLER */}
                    <div className="flex items-center gap-2 py-2 border-t border-black/[0.06] text-sm">
                      <span className="text-[10px] font-bold text-green-400 uppercase w-11 flex-shrink-0">↗ Aller</span>
                      <span className="font-semibold text-[#6e6e73] text-xs w-8">{flight.departureAirport}</span>
                      <span className="font-mono font-bold text-[#1d1d1f]">{depTime}</span>
                      <span className="text-[#86868b] text-xs">→</span>
                      <span className="font-mono font-bold text-[#1d1d1f]">{arrTime}</span>
                      <span className="font-semibold text-[#6e6e73] text-xs w-8">{flight.arrivalAirport}</span>
                      <span className="ml-auto text-xs text-[#86868b] flex-shrink-0">{fmtShort(flightDepDate)}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 py-2 border-t border-black/[0.06] text-sm">
                      ...
                      <span className="font-semibold text-[#6e6e73] text-xs w-8">{flight.arrivalAirport}</span>
                      <span className="font-mono font-bold text-[#1d1d1f]">{retDepTime || depTime}</span>
                      <span className="text-[#86868b] text-xs">→</span>
                      <span className="font-mono font-bold text-[#1d1d1f]">{retArrTime || arrTime}</span>
                      <span className="font-semibold text-[#6e6e73] text-xs w-8">{flight.departureAirport}</span>
                      <span className="ml-auto text-xs text-[#86868b] flex-shrink-0">{fmtShort(flight.returnDate)}</span>
                    </div>

                    {isSelected && (
                      <div className="mt-2 pt-2 border-t border-black/[0.06] text-xs font-semibold text-blue-400 flex items-center gap-1">
            ...
            <div className="text-center py-10 text-[#86868b]">
              ...
              <p className="text-xs mt-1 text-[#d2d2d7]">Vols directs et avec correspondances disponibles</p>
            </div>
          )}
        </div>
      ),
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* ── Hero ──────────────────────────────────────────────────────────────── */}
      <div className="relative h-72 md:h-[420px] overflow-hidden bg-[#f5f5f7]">
        {event.image ? (
        ...
        <div className="absolute top-6 left-4 md:left-8">
          <Link href="/search" className="inline-flex items-center gap-1.5 text-[#1d1d1f]/60 hover:text-[#1d1d1f] bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-lg text-sm transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {event.description && (
          <p className="text-[#6e6e73] mb-5 max-w-3xl leading-relaxed text-sm">{event.description}</p>
        )}
        ...
        {(event as any).venueInfo && (() => {
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8 p-4 bg-[#f5f5f7] border border-black/[0.06] rounded-2xl">
              <div className="flex items-start gap-2.5">
                <Building2 className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-[11px] text-[#86868b] font-semibold uppercase tracking-wide mb-0.5">Type de lieu</p>
                  <p className="text-sm text-[#1d1d1f]">{vi.type}</p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <Navigation className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-[11px] text-[#86868b] font-semibold uppercase tracking-wide mb-0.5">Localisation</p>
                  <p className="text-sm text-[#1d1d1f]">{vi.location}</p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <Bus className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-[11px] text-[#86868b] font-semibold uppercase tracking-wide mb-0.5">Transports</p>
                  <ul className="space-y-0.5">
                    {vi.transport.map((t, i) => <li key={i} className="text-sm text-[#1d1d1f]">{t}</li>)}
                  </ul>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-[11px] text-[#86868b] font-semibold uppercase tracking-wide mb-0.5">Parking</p>
                  <p className="text-sm text-[#1d1d1f]">{vi.parking}</p>
                </div>
              </div>
            </div>
          );
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="bg-[#f5f5f7] border border-black/[0.08] rounded-xl p-4 text-center">
            <Ticket className="w-5 h-5 text-blue-400 mx-auto mb-1" />
            <p className="text-xs text-[#86868b] mb-0.5">Billets dès</p>
            <p className="text-lg font-bold text-[#1d1d1f]">{formatPrice(event.minPrice > 0 ? event.minPrice : 45)}</p>
          </div>
          <div className="bg-[#f5f5f7] border border-black/[0.08] rounded-xl p-4 text-center">
            <HotelIcon className="w-5 h-5 text-purple-400 mx-auto mb-1" />
            <p className="text-xs text-[#86868b] mb-0.5">Hôtel dès</p>
            <p className="text-lg font-bold text-[#1d1d1f]">{hotels.length > 0 ? formatPrice(Math.min(...hotels.map(h => h.price))) : "~€89"}<span className="text-xs text-[#86868b]">/nuit</span></p>
          </div>
          <div className="bg-[#f5f5f7] border border-black/[0.08] rounded-xl p-4 text-center">
            <Plane className="w-5 h-5 text-green-400 mx-auto mb-1" />
            <p className="text-xs text-[#86868b] mb-0.5">Vol depuis</p>
            <p className="text-lg font-bold text-[#1d1d1f]">{flights.length > 0 ? formatPrice(Math.min(...flights.map(f => f.price))) : "~€49"}</p>
          </div>
        </div>
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-[#f5f5f7] border border-black/[0.08] rounded-2xl overflow-hidden"
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                        step.selected ? "bg-blue-600 text-[#1d1d1f]" : "bg-[#f5f5f7] text-blue-400"
                      }`}>
                        {step.selected ? <CheckCircle className="w-5 h-5" /> : <Icon className="w-5 h-5" />}
                      </div>
                      <div className="text-left">
                        <span className="font-bold text-[#1d1d1f]">{step.title}</span>
