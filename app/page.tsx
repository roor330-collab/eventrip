...
const CATEGORIES = [
  { label: "Concerts", emoji: "🎵", desc: "Pop, Rock, Hip-Hop, Électro", type: "music", bg: "bg-purple-600/10 hover:bg-purple-600/20", border: "border-purple-500/30", text: "text-purple-300" },
  { label: "Football", emoji: "⚽", desc: "Liga, Serie A, Bundesliga, Ligue 1", type: "sports", bg: "bg-green-600/10 hover:bg-green-600/20", border: "border-green-500/30", text: "text-green-300" },
  { label: "Festivals", emoji: "🎪", desc: "Lollapalooza, Primavera, Rock am Ring", type: "festival", bg: "bg-orange-600/10 hover:bg-orange-600/20", border: "border-orange-500/30", text: "text-orange-300" },
];
    ...
    fetch(`/api/events?${params}`)
      ...
      .then(data => {
        if (data.success && data.events?.length) {
          ...
          setEvents(activeCategory === "festival" ? evts.filter(e => e.type === "festival").slice(0, 9) : evts.slice(0, 9));
        } else setEvents([]);
      })
      .catch(() => setEvents([]))
      ...
      <section className="relative min-h-[92vh] flex flex-col justify-center items-center overflow-hidden pt-16">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}
            ...
            Voyagez pour{" "}
            <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">la passion</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }}
            className="text-lg md:text-xl text-white/50 max-w-2xl mx-auto mb-12">
            Billets · Vol · Hôtel — un seul pack, un seul paiement. Le <em className="text-white/70 not-italic font-medium">Gig Tripping</em> sans prise de tête.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }} className="mb-8">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="flex flex-wrap justify-center gap-3">
            {COUNTRIES.map(c => (
              <Link key={c.code} href={`/search?country=${c.code}`}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
            {CATEGORIES.map((cat, i) => (
              <motion.button key={cat.type} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                {activeCategory === cat.type && <div className="absolute top-3 right-3 w-5 h-5 bg-white rounded-full flex items-center justify-center"><div className="w-2 h-2 bg-blue-600 rounded-full" /></div>}
              </motion.button>
            ))}
          </div>
          {loading ? (
            <div className="flex justify-center py-16"><Loader2 className="w-10 h-10 animate-spin text-white/30" /></div>
          ) : events.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {events.map((event, idx) => (
                  <motion.div key={event.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.05 }}>
                    <DarkEventCard event={event} />
                  </motion.div>
                ))}
              </div>
              <div className="text-center mt-10">
                <Link href="/search"><button className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-white font-medium transition-all">Voir tous les événements <ArrowRight className="w-4 h-4" /></button></Link>
              </div>
            </>
          ) : (
            <div className="text-center py-16 text-white/30"><Ticket className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Aucun événement dans cette catégorie.</p></div>
          )}
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {HOW_IT_WORKS.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="relative">
                ...
                {i < HOW_IT_WORKS.length - 1 && <div className="hidden md:block absolute top-12 -right-4 text-white/10"><ChevronRight className="w-8 h-8" /></div>}
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, scale: 0.97 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            ...
            <Link href="/search"><button className="inline-flex items-center gap-3 px-10 py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold text-lg transition-all shadow-lg shadow-blue-900/30">Explorer les événements <ArrowRight className="w-5 h-5" /></button></Link>
          </motion.div>
        </div>
      </section>
    ...
    <Link href={`/event/${event.id}`} className="block group">
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/8 hover:border-white/20 transition-all duration-300">
        <div className="relative h-44 bg-white/5 overflow-hidden">
          ...
          <div className={`absolute top-3 right-3 ${typeBadge[event.type] || "bg-blue-600/80"} backdrop-blur-sm text-white px-2.5 py-1 rounded-lg text-xs font-semibold`}>{typeLabel[event.type] || event.type}</div>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-400 font-bold text-base">dès {event.minPrice > 0 ? event.minPrice : 45}€</p>
            </div>
            <span className="text-xs text-white/30 group-hover:text-white/60 flex items-center gap-1 transition-colors">Voir le pack <ChevronRight className="w-3.5 h-3.5" /></span>
          </div>
        </div>
      </div>
    </Link>
